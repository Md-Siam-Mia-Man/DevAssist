...
async function handleRemoveComments(files, options) {
  ...
  await walkDir(projectDir, config, (filePath) => {
    ...
    try {
      ...
      const formatted = formatCode(content, filePath);
      if (content !== formatted) {
        fs.writeFileSync(filePath, formatted, "utf8");
        console.log(
          kleur.green(`✔ Cleaned: ${path.relative(projectDir, filePath)}`),
        );
        modifiedCount++;
      }
      processedCount++;
      ...
      console.error(
        kleur.red(
          `✖ Error processing ${path.relative(projectDir, filePath)}: ${err.message}`,
        ),
      );
    }
  });
  ...
  console.log(
    `📊 Stats:  ${processedCount} files scanned, ${modifiedCount} files modified.`,
  );
  console.log("");
}
